...
def doc_index_retrieve(query_str, retriever):
...
def create_benchmark_df(query_str, memory_retrieval_time, index_retrieval_time, time_to_result):
    return None

#for whatever reason, this wrapper is needed for the pipeline to only return one output
def get_model_output(response):#, benchmark_df):
    #output = {"response": response, "benchmark_df": "test"}
    #print(f"DEBUG {type(output)}")
    return response 

def output_fr(output_dict):
    return output_dict
#Return pandas object that can be appended to csv with desired data 
...
def build_and_run_pipeline(doc_index, pipeline_memory, queries):
    ...
    pipeline = QueryPipeline(
        modules={
            ...
            #"benchmark_df": FnComponent(create_benchmark_df),
            #"output": FnComponent(get_model_output),
            #"output_fr": FnComponent(output_fr)
        },
        verbose=True
    )
